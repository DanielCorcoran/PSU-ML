...
def make_data():
    ...
    '''
    with open('GMM_dataset.txt') as f:
        data = []
        for line in f:
            x, y = line.split()
...
def k_means(data):
    ...
    '''
    #Run on a range from 2 to 6 clusters
    for k in range(2, 7):
        sse = []
        final_clusters = []
        final_centroids = []
        ...
        print('Cluster SSE when k = ' + str(k) + ': ' + str(min(sse)))
        colors = ['r', 'g', 'b', 'y', 'c', 'm', 'k']
        for cluster in final_clusters[min_sse_index]:
            x = [x[0] for x in cluster]
            y = [y[1] for y in cluster]
            plt.scatter(x, y, color = colors[final_clusters[min_sse_index].index(cluster)])
